import React, { useState,useEffect } from 'react';
import { Text,View,TextInput,StyleSheet,ScrollView,TouchableOpacity,Image,Pressable } from "react-native";
import database from '@react-native-firebase/database';

export default function Admin({navigation}){ 
    let [course,setCourse] = useState()
    let [teacher,setTeacher] = useState()
    let [duration,setDuration] = useState()
    let [timing,setTiming] = useState()
    const add=async()=>{
        console.log(course)
        console.log(teacher)
        console.log(duration)
        console.log(timing)
        
       
        
    
    await  database().ref("courses").child("course").set({
        Course:course,
        Teacher:teacher,
        Duration:duration,
        Timing:timing
    })
       .then(() => {
     
         alert("Your added sucessfully")
         setCourse()
         setTeacher()
         setDuration()
         setTiming()

       })
       .catch((error) => {
         const errorCode = error.code;
         const errorMessage = error.message;
         // ..
       });
    }

//   };
    



   

    return(
            <View style={{backgroundColor: '#FFFFFF',height:1000}}>
                <Image style={styles.logo} source={{uri:'data:image/png;base64,'}}/>
                <Text style={styles.text}>Admin Panel</Text>
               <TextInput style={styles.input} placeholder="Course Name" value= {course} onChangeText={value=>setCourse (value)}/> 
               <TextInput style={styles.input} placeholder="Teacher's Name" value= {teacher} onChangeText={value=>setTeacher (value)}/> 
               <TextInput style={styles.input} placeholder="Duration" value= {duration} onChangeText={value=>setDuration (value)} /> 
               <TextInput style={styles.input} placeholder="Course's Timing" value= {timing} onChangeText={value=>setTiming (value)}/> 
               <TouchableOpacity style={styles.button} onPress={()=>add()}><Text style={styles.buttonText}>Add Course</Text></TouchableOpacity>
               <TouchableOpacity style={styles.button} onPress={()=>navigation.navigate("User")}><Text style={styles.buttonText}>See Available Courses</Text></TouchableOpacity>
               
               

             
            </View>
            
        

    );

}
const styles = StyleSheet.create({
    text: {
        color:"black",
        fontSize:30,
        textAlign:"center",
        
      },
      input: {
        height: 50,
        margin: 12,
        borderWidth: 1,
        padding: 10,
        backgroundColor:"grey",
        borderRadius: 10,
      },
      buttonText:{
          backgroundColor:"#205375",
          textAlign:"center",
          color:"#EFEFEF",
          fontSize:25,
          padding:10,
          borderRadius:15
        },
        button:{
            margin:30,
        },
        logo:{
            height:100,
            width:200,
            marginHorizontal:75,
            marginTop:20
        }
    
    
  });


